    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                err = "Incorrect Login Information Make Sure This is the Correct Login"
    else:
...
def login_judge(request):
    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            type = None
            judges = Judge.objects.filter((Q(session__competition__date__gte=datetime.datetime.now() - datetime.timedelta(days=1)) | Q(session__test=True)) & Q(session__active=True)) #got possible judges
            if len(judges) > 0:
...
def login_coach(request):
    ...
    if request.method == 'POST':
        ...
        if login_form.is_valid():
            ...
            password = login_form.cleaned_data.get('password')
            type = None
            teams = Team.objects.filter((Q(session__competition__date__gte=datetime.datetime.now() - datetime.timedelta(days=1)) | Q(session__test=True)) & Q(session__active=True))  #got possible teams
            if len(teams) > 0:
